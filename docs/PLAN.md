...
>   arm64 build and a device, none of which exist in the environment this
>   was built in. Everything around it — the prompt, the output guard, the
>   memory decision, storage, display — is built and tested without one,
>   and `NoEngineNarrator` returns null honestly rather than pretending.
>   Swapping in a real engine is one line in `Narrators.forDevice`.
