...
| Force-stop right after **DataStore-merge**, relaunch, verify resume with no duplicates | `RestoreCoordinatorTest.\`interrupted right after DATASTORES_MERGED resumes to the same end state as a clean run\`` (`app/src/test/java/org/mindanchor/continuity/RestoreCoordinatorTest.kt`) | **Proven at the JVM-fake level only.** `RestoreResumeTest.kt` (the real-Room/real-DataStore androidTest class) does **not** have a `DATASTORES_MERGED`-interruption case — it stops at `ROOM_MERGED`. This is a real, narrow gap: the on-device DataStore-write-then-crash interleaving is untested against real DataStore I/O. Worth a real device or a real process-killed test run. |
