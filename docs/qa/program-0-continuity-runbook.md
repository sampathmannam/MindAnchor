...
| Upload a deliberately corrupted `Latest` file; verify restore falls back to the newest decryptable versioned snapshot | `RestoreCandidateSelectorTest.kt` (`app/src/test/java/org/mindanchor/continuity/`) — a corrupt or missing `Latest` falls back to the newest decryptable versioned snapshot (proving newest-to-oldest try order when multiple candidates are corrupt), and a wrong recovery key stops immediately without ever fetching a versioned snapshot (proven by call-count, not just the result value) | **Already proven at the JVM-fake level.** This manual step re-confirms against a real corrupted object in a real Drive account and the real on-screen "Latest was unreadable" fallback affordance, not first-time coverage. |
