...
| Research ledger events (Program 1) | `app/src/main/java/org/mindanchor/data/db/ResearchEntities.kt` (`ResearchLedgerEventEntity`), `app/src/main/java/org/mindanchor/research/ResearchLedgerRepository.kt` | `ContinuitySnapshotRepository.capture` reads `ResearchDao.ledgerEventsNow()` into `ContinuityPayload.researchLedgerEvents` (`ResearchLedgerEventDto`). `RestoreCoordinator.mergeResearchRows` applies them with `INSERT OR IGNORE` on the content-addressed id and then asserts every row of the payload is present, so a re-run after an interrupted restore is a no-op rather than a failure while a genuinely dropped row still fails loudly. The rows are inside the version-2 content hash; a version-1 checkpoint is hashed without them (`ContinuityContentHasher`). |
| Study phases (Program 1) | `app/src/main/java/org/mindanchor/data/db/ResearchEntities.kt` (`StudyPhaseEntity`), `app/src/main/java/org/mindanchor/research/ResearchProvenanceCoordinator.kt` | `ContinuitySnapshotRepository.capture` reads `ResearchDao.studyPhasesNow()` into `ContinuityPayload.studyPhases` (`StudyPhaseDto`); restore applies each with `INSERT OR IGNORE` on the content-addressed id and then asserts presence, because a dropped phase would leave its events pointing at nothing. Both tables are append-only at the database level, so restore never rewrites a row. |
...
## Program 1 confirmation

Program 1 added two durable tables, `research_ledger_events` and `study_phases`, both listed above. The restore preflight was widened to require both to be empty: merging a backup's chain into a chain this phone had already started would fork it, and append-only tables cannot be un-forked.
